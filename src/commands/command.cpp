...
CommandResponse Echo::run(const Arguments& args,
  ...
  return CommandResponse{.output = out, .err = std::nullopt, .status_code = 0};
}
...
CommandResponse Cat::run(const Arguments& args,
  ...
  if (command_args.empty()) {
    return {{}, "Enter the file please", 1};
  }
  ...
  return {{}, fmt::format("Can't open file {}", command_args[0]), 1};
}
...
CommandResponse Wc::run(const Arguments& args,
  ...
  if (pipe_arg) {
    ...
    if (args.empty()) {
      return {std::nullopt, "Enter the file please", 1};
    }
    auto [is_ok, file_data] = utils::read_file(args[0]);
    if (!is_ok) {
      return {std::nullopt, fmt::format("Can't open file {}", args[0]), 1};
    }
